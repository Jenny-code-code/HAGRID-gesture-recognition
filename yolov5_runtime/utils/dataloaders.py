...
class LoadImagesAndLabels(Dataset):
    ...
    def cache_labels(self, path=Path("./labels.cache"), prefix=""):
        ...
        items = zip(self.im_files, self.label_files, repeat(prefix))
        try:
            pool = Pool(NUM_THREADS)
            results = pool.imap(verify_image_label, items)
        except (OSError, PermissionError) as e:
            LOGGER.warning(f"{prefix}WARNING ⚠️ multiprocessing unavailable while caching labels: {e}. Using 1 worker.")
            pool = None
            results = map(verify_image_label, items)
